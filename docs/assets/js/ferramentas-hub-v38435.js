...
function openHub(){const e=$('ferramentasModal');if(e){e.classList.add('open');e.setAttribute('aria-hidden','false');$('ferramentasBusca')?.focus()}}
...
const A={clinometro:clino,geocamera:geocamera,colunas:columns,tempo:deep,gps:()=>openSection(['localizacao','localização']),amostras:()=>openSection(['amostras']),nivel:()=>document.getElementById('abrirNivelArandu')?.click(),bussola:()=>document.getElementById('abrirBussolaArandu')?.click(),estereograma:()=>document.getElementById('abrirEstereogramaArandu')?.click(),calculadora:()=>document.getElementById('abrirCalculadoraEstrutural')?.click(),macrogeo:()=>planned('MacroGeo')};
function filter(){const q=norm($('ferramentasBusca')?.value);let n=0;qa('.ita-tool-card').forEach(c=>{const ok=!q||norm(c.dataset.search+' '+c.textContent).includes(q);c.style.display=ok?'':'none';if(ok)n++});qa('.ita-tools-section').forEach(s=>s.style.display=qa('.ita-tool-card',s).some(c=>c.style.display!=='none')?'':'none');$('ferramentasVazio').style.display=n?'none':'block'}
function wire(){$('abrirFerramentas')?.addEventListener('click',openHub);qa('[data-close="ferramentasModal"]').forEach(b=>b.addEventListener('click',closeHub));qa('[data-tool-action]').forEach(b=>b.addEventListener('click',()=>A[b.dataset.toolAction]?.()));$('ferramentasBusca')?.addEventListener('input',filter);$('ferramentasLimparBusca')?.addEventListener('click',()=>{$('ferramentasBusca').value='';filter();$('ferramentasBusca').focus()})}
