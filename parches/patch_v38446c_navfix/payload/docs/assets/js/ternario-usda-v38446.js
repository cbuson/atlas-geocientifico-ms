...
function wire(){
 ['ternarioSand','ternarioSilt','ternarioClay','ternarioSample'].forEach(id=>$(id)?.addEventListener('input',update));
 $('ternarioNormalize')?.addEventListener('click',normalize);
 $('ternarioClear')?.addEventListener('click',clear);
 const modal=$('ternarioModal');
 if(modal && !modal.dataset.ternarioDelegated){
   modal.dataset.ternarioDelegated='1';
   modal.addEventListener('click',function(e){
     const target=e.target.closest('button,a');
     if(!target || !modal.contains(target))return;
     if(target.matches('[data-ternario-help]')){e.preventDefault();e.stopPropagation();showSubview('help');return}
     if(target.matches('[data-ternario-science]')){e.preventDefault();e.stopPropagation();showSubview('science');return}
     if(target.matches('[data-ternario-return]')){e.preventDefault();e.stopPropagation();returnToDiagram();return}
     if(target.matches('[data-ternario-open-science]')){e.preventDefault();e.stopPropagation();showSubview('science');return}
     if(target.matches('[data-ternario-mode]')){e.preventDefault();setMode(target.dataset.ternarioMode);return}
     if(target.matches('[data-close="ternarioModal"]')){
       e.preventDefault();e.stopPropagation();
       if(returnToDiagram())return;
       close('ternarioModal');document.getElementById('ferramentasModal')?.classList.add('open');return;
     }
   },true);
 }
 document.querySelectorAll('[data-close="ternarioHelpModal"]').forEach(b=>b.addEventListener('click',()=>close('ternarioHelpModal')));
 const c=$('ternarioCanvas');
...
window.ITA_TERNARIO={version:'1.0.3',open,help,science,returnToDiagram,classify,update};
})();
